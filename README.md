...
---
## 2. 核心实验参数与代码实现细节 (Implementation Details)
...
### 2.2 PRP-DQN 模型架构 (1D-CNN Architecture)
...
### 2.3 强化学习训练超参数 (DRL Hyperparameters)
...
### 2.4 优先经验回放池 (PER Buffer)
* **优先级采样指数 (Alpha):** $0.6$
* **经验池容量 (Capacity):** * 高相关(相似)路径组：**10,000**
  * 低相关(孤岛)路径组：**15,000** (为孤岛路径分配更大容量以增强探索)

### 2.5 MI-PSO 优化器参数 (MI-PSO Settings)
* **粒子群规模 (Swarm Size):** $20$
...
## 3. 核心算法与伪代码 (Core Algorithm & Pseudocode)
