...
def execute_Tr(states):
    """
    完全向量化、且修复了 MDP 漏洞的 29 条规则版本。
    states: 形状为 (N, 3) 的 NumPy 数组。
    """
    N = len(states)
    # 提取状态矩阵的 dx, dy, dz 列
    DX = states[:, 0]
    ...
    # 【修复 MDP 漏洞】：移除 random.uniform，改用固定的环境坐标！
    # 如果每次调用这三个坐标都在变，相同的 dx, dy, dz 会得到不同的奖励，DQN 绝对无法收敛。
    current_x, current_y, current_z = 100.0, 100.0, 100.0
    ...
    # 预计算常量布尔条件
    cond_ty_sy = (TARGET_Y > simulated_y)
    cond_tz_cz = (TARGET_Z < current_z)
    cond_tx_cz = (TARGET_X < current_z)
    cond_ty_cz = (TARGET_Y < current_z)
    cond_tz_cx = (TARGET_Z < current_x)
    cond_tz_cy = (TARGET_Z < current_y)

    # 创建形状为 (29, N) 的二维布尔矩阵，记录 N 个样本是否触发了 29 条规则
    rules = np.zeros((29, N), dtype=bool)

    # --- 分支 1-4 (!= 在布尔运算中用 ^ 异或表示) ---
    rules[0] = (abs_dx < MIN_PLANNING_X) ^ (abs_dy < MIN_PLANNING_X)
    rules[1] = (abs_dx < MIN_PLANNING_X) ^ (abs_dz < MIN_PLANNING_X)
    rules[2] = (abs_dx < MIN_PLANNING_X) ^ (abs_dx < MIN_PLANNING_Y)
    rules[3] = (abs_dx < MIN_PLANNING_X) ^ (abs_dx < MIN_PLANNING_Z)

    # --- 分支 5-9 ---
    cond_dz_mpz2 = (abs_dz > MIN_PLANNING_Z * 2)
    rules[4] = cond_dz_mpz2 ^ (abs_dx > MIN_PLANNING_Z * 2)
    rules[5] = cond_dz_mpz2 ^ (abs_dy > MIN_PLANNING_Z * 2)
    rules[6] = cond_dz_mpz2 ^ (abs_dz > MIN_PLANNING_X * 2)
    rules[7] = cond_dz_mpz2 ^ (abs_dz > MIN_PLANNING_Y * 2)
    rules[8] = cond_dz_mpz2 ^ (abs_dz > MIN_PLANNING_Z)

    # --- 分支 10-15 ---
    cond10_left = cond_ty_sy & (DY < 20)
    rules[9]  = cond10_left ^ (cond_ty_sy & (DY < 10))
    rules[10] = cond10_left ^ (cond_ty_sy & (DY < 30))
    rules[11] = cond10_left ^ (cond_ty_sy & (DY < 40))
    rules[12] = cond10_left ^ (cond_ty_sy & (DY < 50))
    rules[13] = cond10_left ^ (cond_ty_sy & (DX < 20))
    rules[14] = cond10_left ^ (cond_ty_sy & (DZ < 20))

    # --- 分支 16-21 ---
    cond16_left = (abs_dy > CRITICAL_X_VELOCITY * 1.5)
    rules[15] = cond16_left ^ (abs_dx > CRITICAL_X_VELOCITY * 1.5)
    rules[16] = cond16_left ^ (abs_dz > CRITICAL_X_VELOCITY * 1.5)
    rules[17] = cond16_left ^ (abs_dy > CRITICAL_X_VELOCITY)
    rules[18] = cond16_left ^ (abs_dy > CRITICAL_X_VELOCITY * 2)
    rules[19] = cond16_left ^ (abs_dy > CRITICAL_Z_VELOCITY * 1.5)
    rules[20] = cond16_left ^ (abs_dy > CRITICAL_Y_VELOCITY * 1.5)

    # --- 分支 22-29 ---
    cond22_left = cond_tz_cz & (DZ > CRITICAL_Z_VELOCITY)
    rules[21] = cond22_left ^ (cond_tx_cz & (DZ > CRITICAL_Z_VELOCITY))
    rules[22] = cond22_left ^ (cond_ty_cz & (DZ > CRITICAL_Z_VELOCITY))
    rules[23] = cond22_left ^ (cond_tz_cx & (DZ > CRITICAL_Z_VELOCITY))
    rules[24] = cond22_left ^ (cond_tz_cy & (DZ > CRITICAL_Z_VELOCITY))
    rules[25] = cond22_left ^ (cond_tz_cz & (DX > CRITICAL_Z_VELOCITY))
    rules[26] = cond22_left ^ (cond_tz_cz & (DY > CRITICAL_Z_VELOCITY))
    rules[27] = cond22_left ^ (cond_tz_cz & (DZ > CRITICAL_X_VELOCITY))
    rules[28] = cond22_left ^ (cond_tz_cz & (DZ > CRITICAL_Y_VELOCITY))

    # --- 3. 组装结果 ---
    triggered_sets = [set() for _ in range(N)]
    for rule_idx in range(29):
        # 瞬间找到所有触发了当前规则的样本索引
        triggered_indices = np.where(rules[rule_idx])[0]
        rule_id = rule_idx + 1
        for idx in triggered_indices:
            triggered_sets[idx].add(rule_id)

    return triggered_sets
...
def compute_robustness(state, path):
    """计算鲁棒性（输入为原始值）"""
    dx, dy, dz = state
    base = execute_Tr(np.array([[dx, dy, dz]]))[0]
    if not base:
        return 0.0
    ...
    for dw in [-1, 0, 1]:
        for dt in [-1, 0, 1]:
            for dz in [-1, 0, 1]:
                ...
                n_trig = execute_Tr(np.array([[ndx, ndy, ndz]]))[0]
                if not n_trig:
                    continue
...
def generate_samples_for_similar_paths(similar_group, num_candidates=2000, top_k=200, run_id=1):
    ...
    for path_idx in similar_group:
        ...
        while len(candidate_samples) < num_candidates and attempts < num_candidates * 10:
            ...
            triggered = execute_Tr(np.array([[dx, dy, dz]]))[0]
